...
const UploadImgPrincipalV2 = ({ onPrincipalUpdate }) => {
  ...
  const ImageManager = () => {
    ...
    const handleImageClick = (url) => {
      setSelectedImages((prevSelected) => {
        // If the clicked image is already selected, deselect it (set to null).
        if (prevSelected === url) {
          return null;
        } else {
        ...
        <div style={{ display: "flex", gap: "10px", justifyContent: "center" }}>
          {selectedImages
            ? selectedImages.length > 0 && (
                <button
                  onClick={() => {
                    setImagenPrincipal(selectedImages);
                    togglePopup();
                  }}
                  style={{
                    cursor: "pointer",
  ...
  useEffect(() => {
    if (onPrincipalUpdate) {
      onPrincipalUpdate(imagenPrincipal); // Call the callback prop
    }
  }, [imagenPrincipal]);
...
export default UploadImgPrincipalV2;
